...
class Summary(CoupangWing):
    """쿠팡 로켓그로스 정산현황의 수익 현황을 조회하는 클래스.

    - **Menu**: 정산 > 로켓그로스 정산현황 > 홈
    - **API**: https://wing.coupang.com/tenants/rfm/v2/settlements/profit-status/search
    - **Referer**: https://wing.coupang.com/tenants/rfm/settlements/home
    ...
    **NOTE** 인스턴스 생성 시 `cookies` 인자로 `XSRF-TOKEN` 값이 포함된 쿠키 문자열을 반드시 전달해야 한다.
    ...
    def extract(self, start_from: str, end_to: str, **kwargs) -> dict:
        """로켓그로스 정산현황의 기간 내 수익 현황을 조회해 JSON 형식으로 반환한다.
...
class RocketSettlementDownload(RocketSettlement):
    ...
    **NOTE** 인스턴스 생성 시 `cookies` 인자로 `XSRF-TOKEN` 값이 포함된 쿠키 문자열을 반드시 전달해야 한다.
    ...
    def extract(
        ...
        """로켓그로스 정산현황의 정산 리포트를 요청하고 엑셀 파일로 다운로드한다.

        **NOTE** 2가지 리포트 유형(`report_type`)에 대한 정산 리포트를 각각 다운로드한다.
        ...
        progress: bool
            정산 리포트 목록에 대한 다운로드 진행도 출력 여부. 기본값은 `True`
        ...
        for report in tqdm(response["settlementStatusReports"], desc=f"Downloading reports", disable=(not progress)):
            # 쿠팡 로켓그로스의 정산은 70% 선지급 후 30%를 나중에 지급한다.
            # 각각의 지급건에 대해 정산 리포트가 생성되지만,
            # 주문 내역은 동일하므로 `settlementGroupKey`를 기준으로 중복 처리한다.
    ...
    def download(
        ...
        """정산 리포트 유형(`report_type`)에 대한 엑셀 파일을 생성하고 다운로드한다."""
