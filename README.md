...
- [Airflow Trigger UI](#airflow-trigger-ui)
- [빠른 시작](#빠른-시작)
- [예시 코드](#예시-코드)
...
현재 `linkmerce` 패키지 버전은 `1.0.10`이다.
...
5. `airflow_trigger`: 쇼핑몰 운영 담당자가 Airflow Dag을 수동으로 트리거할 때
    사용하는 Streamlit UI와 Google Sheets 링크/버튼용 FastAPI 서버
...
Airflow Dag은 대부분 `dual_load` 플러그인을 통해 PostgreSQL 적재를 먼저 수행하고,
성공하면 BigQuery 적재를 이어서 수행한다.
PostgreSQL은 더 엄격한 기본키와 타입 제약을 검증하는 1차 적재 대상으로 사용하고,
BigQuery는 기존 분석 테이블 적재 대상으로 유지한다.
...
`airflow_trigger/` 경로는 Airflow REST API를 호출하여 Dag을 수동으로 실행하기 위한 보조 UI를 제공한다.
Airflow UI에 직접 접근하지 않는 운영 담당자나 Google Sheets 기반 작업에서 필요한 Dag만 안전하게 실행하도록 분리한 도구다.

### FastAPI

`airflow_trigger/fastapi/app.py`는 로컬 네트워크의 다른 컴퓨터나 Google Sheets 링크/버튼에서
특정 Dag을 실행하기 위한 작은 HTTP 서버다. 브라우저에서 `/trigger?dag_id=...` 주소로 접속하면 로딩 화면을 표시하고,
Dag run이 종료되면 안내 팝업을 보여준 뒤 페이지 종료를 시도한다.

화면에 노출할 작업명은 `dag_name` 파라미터로 지정할 수 있으며, 실제 실행 대상은 항상 `dag_id`로 결정한다.

```text
http://localhost:16160/trigger?dag_id=gsheets_opex&dag_name=운영비용%20업데이트
```

프로그램에서 JSON 응답이 필요하면 `/api/trigger`를 사용한다. 환경변수, 허용 DAG 목록, Docker 실행 방법은
별도의 [문서](https://github.com/minyeamer/linkmerce/blob/main/airflow_trigger/fastapi/README.md)를 참고한다.
...
`airflow_trigger/streamlit/app.py`는 작업 일시 범위와 차수를 선택해서
`sabangnet_order` Dag run을 생성하는 운영용 UI다. Streamlit secrets에서 Airflow 접속 정보를 읽고,
`/auth/token`으로 access token을 발급받은 뒤 `/api/v2/dags/{dag_id}/dagRuns` 엔드포인트를 호출한다.

같은 `logical_date`의 기존 Dag run이 있으면 삭제한 뒤 새로 실행하므로,
같은 조건의 수동 실행을 다시 요청할 수 있다. Docker 실행, secrets 설정, 입력값과 Dag run 생성 규칙은
별도의 [문서](https://github.com/minyeamer/linkmerce/blob/main/airflow_trigger/streamlit/README.md)를 참고한다.
