...
sabangnet_sold_qty_daily AS (
  ...
  WHERE (order_date
      BETWEEN DATE_SUB(DATE('{{ var("ds_start_date") }}'), INTERVAL 60 DAY)
...
-- Step 3: calculate rolling 60 days quantity

sold_qty_daily_60d AS (
  SELECT
      dates.ymd
    , qty.product_id
    , SUM(qty.sold_qty) AS sold_qty_60d
    , SUM(qty.sabangnet__sold_qty) AS sabangnet__sold_qty_60d
    , SUM(qty.cj_eflexs__sold_qty) AS cj_eflexs__sold_qty_60d
    , SUM(qty.coupang_rfm__sold_qty) AS coupang_rfm__sold_qty_60d
  FROM ds_date_range AS dates
  INNER JOIN sold_qty_daily AS qty
    ON qty.order_date
      BETWEEN DATE_SUB(dates.ymd, INTERVAL 60 DAY)
      AND DATE_SUB(dates.ymd, INTERVAL 1 DAY)
  GROUP BY dates.ymd, qty.product_id
)

SELECT * FROM sold_qty_daily_60d
