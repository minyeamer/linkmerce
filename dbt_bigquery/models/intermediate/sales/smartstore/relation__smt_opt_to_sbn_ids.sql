...
option_type_1 AS (
  ...
  INNER JOIN {{ source('sabangnet', 'mapping_name') }} AS map
    ON (map.shop_id = 'shop0055')
      ...
      AND (CASE
        WHEN map.sku_name LIKE '%$수량$%' THEN REGEXP_CONTAINS(
            opt.option_name
          , REPLACE(
              REGEXP_REPLACE(
                REGEXP_REPLACE(
...
auto_matched_bundle_ids AS (
  SELECT
      smt.option_id_smt
    , smt.option_id_sbn
    , REGEXP_REPLACE(COALESCE(opt.bundle_option_ids, CONCAT(smt.option_id_sbn, ':1')), '-[0-9]{4}', '') AS bundle_product_ids
  FROM (
...
QUALIFY ROW_NUMBER() OVER (
  ...
  ORDER BY
      ARRAY_LENGTH(SPLIT(bundle_product_ids, ',')) ASC
    , option_id_sbn ASC
) = 1
