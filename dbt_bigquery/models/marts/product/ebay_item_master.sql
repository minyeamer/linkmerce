...
ebay_master_item AS (
  SELECT
      item_id
    , MAX(IF(site_type = 2, site_item_id, NULL)) AS item_id_gmkt
    , MAX(IF(site_type = 1, site_item_id, NULL)) AS item_id_iac
    , MAX(item_name) AS item_name
    , MAX(IF(site_type = 2, option_name, NULL)) AS option_name_gmkt
    , MAX(IF(site_type = 1, option_name, NULL)) AS option_name_iac
    , MAX(brand_name) AS brand_name
    , MAX(category_name) AS category_name
    , MAX(IF(site_type = 2, sell_status, NULL)) AS sell_status_gmkt
    , MAX(IF(site_type = 1, sell_status, NULL)) AS sell_status_iac
    , MAX(image_url) AS image_url
    , MAX(IF(site_type = 2, price, NULL)) AS price_gmkt
    , MAX(IF(site_type = 1, price, NULL)) AS price_iac
