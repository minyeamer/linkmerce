{{
  config(
    ...
    meta = {
      'params': [
        {'name': 'DS_START_DATETIME', 'type': 'timestamp'},
        {'name': 'DS_END_DATETIME', 'type': 'timestamp'}
      ]
    },
    schema = 'analytics',
    alias = 'total_order'
...
#} sabangnet__order_status AS (
  ...
  WHERE cor.order_date BETWEEN DS_START_DATETIME::date AND DS_END_DATETIME::date
...
#} sabangnet__product_order_with_cost_data AS (
  ...
  FROM (
    SELECT
      ...
      , ord.order_dt
    FROM (
      (SELECT * FROM sabangnet__bundle_product_order WHERE bundle_option_ids IS NULL)
      UNION ALL
...
#} sabangnet__product_order_with_cj_delivery AS (
  SELECT
    ...
    , ord.cost_amount
  FROM (
    (SELECT * FROM sabangnet__product_order_with_cost_data WHERE bundle_product_count = 1)
    UNION ALL
    (SELECT * FROM sabangnet__product_order_with_split_amount)
  ) AS ord
  LEFT JOIN {{ ref('cj__invoice') }}(
      DS_START_DATETIME::date - 7
    , DS_END_DATETIME::date + 7
  ) AS cj_inv
...
#} sabangnet__product_order AS (
  SELECT
    ...
    , order_dt
  FROM (
    (SELECT * FROM sabangnet__product_order_with_max_delivery WHERE bundle_invoice_count = 1)
    UNION ALL
...
#} smartstore__product_order_with_cj_delivery AS (
  SELECT
    ...
    , ord.cost_amount
  FROM (
    (SELECT * FROM smartstore__exploded_product_order WHERE bundle_product_count = 1)
    UNION ALL
...
#} smartstore__product_order AS (
  SELECT
    ...
    , order_dt
  FROM (
    (SELECT * FROM smartstore__product_order_with_max_delivery WHERE bundle_invoice_count = 1)
    UNION ALL
...
#} coupang_rfm__rocket_sales AS (
  ...
  WHERE sales_date BETWEEN DS_START_DATETIME::date AND DS_END_DATETIME::date
...
#} coupang_rfm__bundle_product_order AS (
  SELECT
    ...
    , ord.delivery_fee
    -- Sales partition key
    , ord.sales_date::timestamp without time zone AS order_dt
...
#} coupang_rfm__product_order AS (
  SELECT
      order_id::text AS order_id
    , option_id::text AS product_order_id
    , product_id
    , 'shop9001' AS shop_id
    , order_status
    , sku_quantity
    ...
    , order_dt
  FROM (
    (SELECT * FROM coupang_rfm__product_order_with_delivery_extra WHERE bundle_product_count = 1)
    UNION ALL
