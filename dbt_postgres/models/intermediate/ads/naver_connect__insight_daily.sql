...
#} sales_daily AS (
  ...
  WHERE sales.order_date BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
),{#

-- Step 3: aggregate naver connect sales by dimensions

#} bundle_product_sales AS (
  SELECT
      space_id
    , mall_product_id
...
#} exploded_product_sales AS (
  ...
  FROM (
    SELECT
      ...
      , cardinality(string_to_array(sales.bundle_product_ids, ',')) AS bundle_product_count
    FROM bundle_product_sales AS sales
    CROSS JOIN LATERAL (
      SELECT bundle_product_id, bundle_product_offset
      FROM unnest(string_to_array(sales.bundle_product_ids, ','))
...
#} SELECT * FROM exploded_product_sales
