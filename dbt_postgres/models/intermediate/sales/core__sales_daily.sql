{{
  config(
    materialized = 'partitioned_table',
    schema = 'xfm_sales',
    partition_by = {
      "field": "order_date",
      "data_type": "date",
      "granularity": "day"
    },
    partitions = pg_date_partitions('ds_start_date', 'ds_end_date')
  )
}}

WITH{#

-- Step 1: prepare sales data

#} sabangnet_sales_daily AS (
  SELECT
      shop_id
    , account_no::text AS account_no
    , product_id
    , order_status
    , sku_quantity
    , payment_amount
    , supply_amount
    , supply_cost
    , delivery_fee
    , NULL::integer AS ad_cost
    , NULL::integer AS extra_cost
    , order_date
  FROM {{ ref('sabangnet__sales_daily') }}
  WHERE order_date BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
),{#

#} smartstore_sales_daily AS (
  SELECT
      (CASE WHEN delivery_type = 7 THEN 'shop9000' ELSE 'shop0055' END) AS shop_id
    , channel_seq::text AS account_no
    , product_id
    , order_status
    , sku_quantity
    , payment_amount
    , supply_amount
    , supply_cost
    , delivery_fee
    , NULL::integer AS ad_cost
    , NULL::integer AS extra_cost
    , order_date
  FROM {{ ref('smartstore__sales_daily') }}
  WHERE order_date BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
),{#

#} coupang_rfm_sales_daily AS (
  SELECT
      'shop9001' AS shop_id
    , vendor_id AS account_no
    , product_id
    , order_status
    , sku_quantity
    , payment_amount
    , supply_amount
    , supply_cost
    , delivery_fee
    , NULL::integer AS ad_cost
    , NULL::integer AS extra_cost
    , order_date
  FROM {{ ref('coupang_rfm__sales_daily') }}
  WHERE order_date BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
),{#

#} extra_sales_daily AS (
  SELECT
      shop_id
    , '0' AS account_no
    , product_id
    , 0 AS order_status
    , NULL::integer AS sku_quantity
    , sales_amount AS payment_amount
    , supply_amount
    , NULL::integer AS supply_cost
    , NULL::integer AS delivery_fee
    , NULL::integer AS ad_cost
    , NULL::integer AS extra_cost
    , sales_date AS order_date
  FROM {{ source('core', 'extra_sales') }}
  WHERE sales_date BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
),{#

-- Step 2: prepare ads data

#} searchad_insight_daily AS (
  SELECT
      'shop0055' AS shop_id
    , customer_id::text AS account_no
    , product_id
    , SUM(ad_cost) AS ad_cost
    , ymd AS order_date
  FROM {{ ref('searchad__insight_daily') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd, customer_id, product_id
),{#

#} searchad_contract_daily AS (
  SELECT
      'shop0055' AS shop_id
    , customer_id::text AS account_no
    , product_id
    , SUM(ad_cost) AS ad_cost
    , ymd AS order_date
  FROM {{ ref('searchad__contract_daily') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd, customer_id, product_id
),{#

#} coupang_ads_insight_daily AS (
  SELECT
      'shop9001' AS shop_id
    , vendor_id AS account_no
    , product_id
    , SUM(ad_cost) AS ad_cost
    , ymd AS order_date
  FROM {{ ref('coupang_ads__insight_daily') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd, vendor_id, product_id
),{#

#} ebay_ads_insight_daily AS (
  SELECT
      (CASE WHEN site_type = 1 THEN 'shop0067' ELSE 'shop0068' END) AS shop_id
    , seller_id AS account_no
    , product_id
    , SUM(ad_cost) AS ad_cost
    , ymd AS order_date
  FROM {{ ref('ebay_ads__insight_daily') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd, site_type, seller_id, product_id
),{#

#} google_ads_insight_daily AS (
  SELECT
      'adop0001' AS shop_id
    , customer_id::text AS account_no
    , product_id
    , SUM(ad_cost) AS ad_cost
    , ymd AS order_date
  FROM {{ ref('google_ads__insight_daily') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd, customer_id, product_id
),{#

#} meta_ads_insight_daily AS (
  SELECT
      'adop0002' AS shop_id
    , account_id AS account_no
    , product_id
    , SUM(ad_cost) AS ad_cost
    , ymd AS order_date
  FROM {{ ref('meta_ads__insight_daily') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd, account_id, product_id
),{#

#} dable_report_daily AS (
  SELECT
      'adop0009' AS shop_id
    , '5000837' AS account_no
    , product_id
    , SUM(ad_cost) AS ad_cost
    , ymd AS order_date
  FROM {{ ref('dable__report_daily') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd, product_id
),{#

#} naver_connect_insight_daily AS (
  SELECT
      'adop0010' AS shop_id
    , space_id::text AS account_no
    , product_id
    , SUM(ad_cost) AS ad_cost
    , ymd AS order_date
  FROM {{ ref('naver_connect__insight_daily') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd, space_id, product_id
),{#

#} tiktok_report_daily AS (
  SELECT
      'adop0006' AS shop_id
    , '5043630' AS account_no
    , product_id
    , SUM(ad_cost) AS ad_cost
    , ymd AS order_date
  FROM {{ ref('tiktok_ads__report_daily') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd, product_id
),{#

#} extra_ads_insight_daily AS (
  SELECT
      shop_id
    , '0' AS account_no
    , brand_id AS product_id
    , ad_cost
    , ymd AS order_date
  FROM {{ source('core', 'extra_ads') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
),{#

-- Step 3: assign searchad cost to the highest-sales shop_id

#} smartstore_product_daily AS (
  SELECT DISTINCT ON (order_date, product_id)
      order_date
    , product_id
    , shop_id
  FROM (
    SELECT
        order_date
      , product_id
      , shop_id
      , SUM(payment_amount) AS payment_amount
    FROM smartstore_sales_daily
    GROUP BY order_date, product_id, shop_id
  ) AS t_
  -- Step 3-1: find the highest-sales smartstore shop_id for each product-day
  ORDER BY order_date, product_id, payment_amount DESC, shop_id ASC
),{#

#} smartstore_brand_daily AS (
  SELECT DISTINCT ON (order_date, brand_name)
      order_date
    , brand_name
    , shop_id
  FROM (
    SELECT
        smt.order_date
      , prd.brand_name
      , smt.shop_id
      , SUM(smt.payment_amount) AS payment_amount
    FROM smartstore_sales_daily AS smt
    INNER JOIN {{ ref('core__product_master') }} AS prd
      ON NULLIF(smt.product_id, '200000') = prd.product_id
    GROUP BY smt.order_date, prd.brand_name, smt.shop_id
  ) AS t_
  -- Step 3-2: find the highest-sales smartstore shop_id for each brand-day
  ORDER BY order_date, brand_name, payment_amount DESC, shop_id ASC
),{#

#} searchad_insight_daily_with_shop_mapping AS (
  SELECT
      COALESCE(prd.shop_id, brd.shop_id, ads.shop_id) AS shop_id
    , ads.account_no
    , ads.product_id
    , ads.ad_cost
    , ads.order_date
  FROM (
    (SELECT * FROM searchad_insight_daily)
    UNION ALL
    (SELECT * FROM searchad_contract_daily)
  ) AS ads
  LEFT JOIN {{ ref('core__product_master') }} AS itm
    ON ads.product_id = itm.product_id
  -- Step 3-3: map searchad cost to shop_id using product first, then brand fallback
  LEFT JOIN smartstore_product_daily AS prd
    ON ads.order_date = prd.order_date AND ads.product_id = prd.product_id
  LEFT JOIN smartstore_brand_daily AS brd
    ON ads.order_date = brd.order_date AND itm.brand_name = brd.brand_name
),{#

-- Step 4: assign coupang-ads cost to the highest-sales shop_id

#} coupang_product_daily AS (
  SELECT DISTINCT ON (order_date, product_id)
      order_date
    , product_id
    , shop_id
  FROM (
    SELECT
        order_date
      , product_id
      , shop_id
      , SUM(payment_amount) AS payment_amount
    FROM (
      (SELECT * FROM coupang_rfm_sales_daily)
      UNION ALL
      (SELECT * FROM sabangnet_sales_daily WHERE shop_id = 'shop0075')
    ) AS t_
    GROUP BY order_date, product_id, shop_id
  ) AS t_
  -- Step 4-1: find the highest-sales coupang shop_id for each product-day
  ORDER BY order_date, product_id, payment_amount DESC, shop_id DESC
),{#

#} coupang_brand_daily AS (
  SELECT DISTINCT ON (order_date, brand_name)
      order_date
    , brand_name
    , shop_id
  FROM (
    SELECT
        cpg.order_date
      , prd.brand_name
      , cpg.shop_id
      , SUM(cpg.payment_amount) AS payment_amount
    FROM (
      (SELECT * FROM coupang_rfm_sales_daily)
      UNION ALL
      (SELECT * FROM sabangnet_sales_daily WHERE shop_id = 'shop0075')
    ) AS cpg
    INNER JOIN {{ ref('core__product_master') }} AS prd
      ON NULLIF(cpg.product_id, '200000') = prd.product_id
    GROUP BY cpg.order_date, prd.brand_name, cpg.shop_id
  ) AS t_
  -- Step 4-2: find the highest-sales coupang shop_id for each brand-day
  ORDER BY order_date, brand_name, payment_amount DESC, shop_id DESC
),{#

#} coupang_ads_insight_daily_with_shop_mapping AS (
  SELECT
      COALESCE(prd.shop_id, brd.shop_id, ads.shop_id) AS shop_id
    , ads.account_no
    , ads.product_id
    , ads.ad_cost
    , ads.order_date
  FROM coupang_ads_insight_daily AS ads
  LEFT JOIN {{ ref('core__product_master') }} AS itm
    ON ads.product_id = itm.product_id
  -- Step 4-3: map coupang ads cost to shop_id using product first, then brand fallback
  LEFT JOIN coupang_product_daily AS prd
    ON ads.order_date = prd.order_date AND ads.product_id = prd.product_id
  LEFT JOIN coupang_brand_daily AS brd
    ON ads.order_date = brd.order_date AND itm.brand_name = brd.brand_name
),{#

-- Step 5: prepare cost data

#} expense_daily AS (
  SELECT
      '200000' AS product_id
    , 'adop0005' AS shop_id
    , SUM(amount) AS extra_cost
    , ymd AS order_date
  FROM {{ source('core', 'expense') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd
),{#

#} opex_daily AS (
  SELECT
      brand_id AS product_id
    , (CASE WHEN dept_id = 1 THEN 'adop0004' ELSE 'adop0003' END) AS shop_id
    , SUM(amount) AS extra_cost
    , ymd AS order_date
  FROM {{ ref('core__opex_daily') }}
  WHERE ymd BETWEEN {{ pg_batch_start_date() }} AND {{ pg_batch_end_date() }}
  GROUP BY ymd, product_id, dept_id
),{#

-- Step 7: concat sales, ads, and cost data

#} insight_daily AS (
  SELECT
      shop_id
    , account_no
    , product_id
    , 8 AS order_status
    , NULL::integer AS sku_quantity
    , NULL::integer AS payment_amount
    , NULL::integer AS supply_amount
    , NULL::integer AS supply_cost
    , NULL::integer AS delivery_fee
    , ad_cost
    , NULL::integer AS extra_cost
    , order_date
  FROM (
    (SELECT * FROM searchad_insight_daily_with_shop_mapping)
    UNION ALL
    (SELECT * FROM coupang_ads_insight_daily_with_shop_mapping)
    UNION ALL
    (SELECT * FROM ebay_ads_insight_daily)
    UNION ALL
    (SELECT * FROM google_ads_insight_daily)
    UNION ALL
    (SELECT * FROM meta_ads_insight_daily)
    UNION ALL
    (SELECT * FROM dable_report_daily)
    UNION ALL
    (SELECT * FROM naver_connect_insight_daily)
    UNION ALL
    (SELECT * FROM tiktok_report_daily)
    UNION ALL
    (SELECT * FROM extra_ads_insight_daily)
  ) AS t_
),{#

#} cost_daily AS (
  SELECT
      shop_id
    , '0' AS account_no
    , product_id
    , 9 AS order_status
    , NULL::integer AS sku_quantity
    , NULL::integer AS payment_amount
    , NULL::integer AS supply_amount
    , NULL::integer AS supply_cost
    , NULL::integer AS delivery_fee
    , NULL::integer AS ad_cost
    , extra_cost
    , order_date
  FROM (
    (SELECT * FROM expense_daily)
    UNION ALL
    (SELECT * FROM opex_daily)
  ) AS t_
),{#

#} sales_daily AS (
  SELECT
      shop_id
    , account_no
    , product_id
    , order_status
    , SUM(sku_quantity) AS sku_quantity
    , SUM(payment_amount) AS payment_amount
    , SUM(supply_amount) AS supply_amount
    , SUM(supply_cost) AS supply_cost
    , SUM(delivery_fee) AS delivery_fee
    , SUM(ad_cost) AS ad_cost
    , SUM(extra_cost) AS extra_cost
    , order_date
  FROM (
    (SELECT * FROM sabangnet_sales_daily)
    UNION ALL
    (SELECT * FROM smartstore_sales_daily)
    UNION ALL
    (SELECT * FROM coupang_rfm_sales_daily)
    UNION ALL
    (SELECT * FROM extra_sales_daily)
    UNION ALL
    (SELECT * FROM insight_daily)
    UNION ALL
    (SELECT * FROM cost_daily)
  )
  GROUP BY order_date, shop_id, account_no, product_id, order_status
){#

#} SELECT * FROM sales_daily
