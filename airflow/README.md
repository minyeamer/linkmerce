...
| `stock_report` | 평일 `17:00`, 트리거 가능 | 재고 내역을 병합 및 엑셀 파일로 변환해 Slack 업로드 |
...
`ds_task_id`를 전달하면 해당 Task의 XCom에서 `ds_start_date`, `ds_end_date`를 꺼내 dbt `vars`로 전달한다. Dag 코드에서는 직접 `vars`를 구성하지 않고 `ds_task_id`를 넘기는 방식을 표준으로 사용한다.
...
즉, test mode에서는 외부 적재, dbt 실행, Slack 파일 업로드를 모두 생략하고 Dag 오케스트레이션과 DuckDB 기준 결과만 확인할 수 있다.
