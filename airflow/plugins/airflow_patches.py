...
def _apply_test_mode_patches() -> None:
    """테스트 모드(`test_mode=true`)일 때 BigQuery/PostgreSQL 적재 및 Slack 알림을 차단하고 미리보기를 출력한다.
    이 함수는 각 태스크 프로세스 시작 시 자동 호출되며, 외부 라이브러리 메서드를 런타임에 패치한다.

    패치 대상:
    - `BigQueryClient`: 소스 테이블 미리보기(LIMIT 5)를 반환하는 함수로 교체
    - `PostgresClient`: 소스 테이블 미리보기(LIMIT 5)를 반환하는 함수로 교체
    ...
    logger.warning("[TEST-MODE] BigQuery/PostgreSQL uploads and Slack notifications are disabled")

    def _simulate_table_upload(self, connection: DuckDBConnection, source_table: str, *args, **kwargs) -> list[dict]:
