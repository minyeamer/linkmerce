"""
# 기타매출 구글시트-DB 동기화 파이프라인

구글 스프레드시트에서 기타매출 데이터를 읽어서 BigQuery와 PostgreSQL에 덮어쓰기로 적재한다.
...
with DAG(
    dag_id = "sync_gsheets__extra_sales",
    ...
    doc_md = __doc__,
    tags = [
        "priority:medium", "platform:gsheets", "objective:sales", "schedule:none",
        "write:overwrite", "upstream:fastapi", "plugin:dbt"
    ],
) as dag:

    PATH = "gsheets.core__extra_sales"
    ...
    def dual_load_from_gsheets(configs: dict) -> dict:
        ...
        return {
            "new_rows": new_rows,
            "new_partitions": sorted({row["sales_date"] for row in new_rows}),
        }
    ...
    dbt_run = [dbt_bigquery_extra_sales_group(), dbt_postgres_extra_sales_group()]
