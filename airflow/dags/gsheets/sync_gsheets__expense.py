"""
# 고정지출 구글시트-DB 동기화 파이프라인

구글 스프레드시트에서 고정지출 데이터를 읽어서 BigQuery와 PostgreSQL에 덮어쓰기로 적재한다.
...
with DAG(
    ...
    doc_md = __doc__,
    tags = [
        "priority:medium", "platform:gsheets", "objective:cost", "schedule:none",
        "write:overwrite", "upstream:fastapi", "plugin:dbt"
    ],
) as dag:

    PATH = "gsheets.core__expense"
    ...
    dbt_run = [dbt_bigquery_expense_group(), dbt_postgres_expense_group()]
