"""
# 주문상태 구글시트-DB 동기화 파이프라인

구글 스프레드시트에서 주문상태 데이터를 읽어서 BigQuery와 PostgreSQL에 덮어쓰기로 적재한다.
...
with DAG(
    dag_id = "sync_gsheets__order_status",
    ...
    PATH = "gsheets.core__order_status"
    ...
    def dual_load_from_gsheets(configs: dict) -> dict:
        ...
        return {
            "new_rows": new_rows,
            "new_partitions": sorted({row["order_date"] for row in new_rows}),
        }
