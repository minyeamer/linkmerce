"""
# 운영비용 구글시트-DB 동기화 파이프라인

구글 스프레드시트에서 운영비용 데이터를 읽어서 BigQuery와 PostgreSQL에 덮어쓰기로 적재한다.
...
with DAG(
    ...
    def dual_load_from_gsheets(configs: dict) -> dict:
        ...
        return {
            "new_rows": new_rows,
            "new_partitions": sorted({row[key] for row in new_rows for key in ["start_date", "end_date"]}),
        }
    ...
    dbt_run = [dbt_bigquery_opex_group(), dbt_postgres_opex_group()]
