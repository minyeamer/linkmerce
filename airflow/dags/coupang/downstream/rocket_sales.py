...
## 변환(Transform)
엑셀 바이너리 형식의 정산 리포트를 JSON 형식으로 변환하고 리포트 유형별 DuckDB 테이블에 적재한다.

## 적재(Load)
각각의 정산 리포트를 대응되는 BigQuery/Postgres 테이블과 MERGE 문으로 병합한다.
...
with DAG(
    ...
    schedule = None, # `coupang` Dag 실행 후 트리거 (0 9 * * *)
    start_date = pendulum.datetime(2025, 10, 22, tz="Asia/Seoul"),
    dagrun_timeout = timedelta(minutes=10),
    catchup = False,
    doc_md = __doc__,
    tags = [
        "priority:high", "platform:coupang-wing", "objective:sales", "credentials:cookies",
        "schedule:daily", "schedule:none", "time:morning", "write:merge",
        "upstream:dagrun"
    ],
) as dag:
