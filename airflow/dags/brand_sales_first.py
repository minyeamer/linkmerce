...
with DAG(
    ...
    def etl_brand_sales(
        ...
        start_date = str(data_interval_end.subtract(days=2).date())
        end_date = str(data_interval_end.subtract(days=1).date())
        project_id = service_account["project_id"]

        with DuckDBConnection(tzinfo="Asia/Seoul") as conn:
            aggregated_sales(cookies, mall_seq, start_date, end_date, connection=conn, how="async", return_type="none")
