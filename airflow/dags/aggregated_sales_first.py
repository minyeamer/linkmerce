...
with DAG(
    ...
    def etl(
        ...
        with DuckDBConnection() as conn:
            ...
            start_date, end_date = data_interval_end.subtract(days=2).date(), data_interval_end.subtract(days=1).date()
            aggregated_sales(cookies, mall_seq, str(start_date), str(end_date), connection=conn, how="async", return_type="none")

            project_id = Variable.get("project_id")
            with connect_bigquery(project_id, Variable.get("service_account")) as client:
                condition = f"WHERE paymentDate BETWEEN '{start_date}' AND '{end_date}'"
                partition_by = dict(by=["paymentDate"])
                overwrite_table_from_duckdb(client, conn, "data", table_info["data"], project_id, condition, _sales_schema(), partition_by)
